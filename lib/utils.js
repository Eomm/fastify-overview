...
function withEmptyPropsHidden (structure, opts) {
  ...
  if (opts?.routesFilter) {
    return withRoutesFiltered(JSON.parse(filterStructure), opts.routesFilter)
  }
...
function withRoutesFiltered (structure, routesFilterPredicate) {
  if (structure.routes) {
    structure.routes = structure.routes.filter(routesFilterPredicate)
  }
  if (structure.children) {
    structure.children = structure.children.map(child => withRoutesFiltered(child, routesFilterPredicate))
  }
  return structure
}
